...
-- Prevent old loops/connections from surviving when the hub is executed again.
local HUB_KEY = "__IKAROHUB_CARROS_STATE"
local previousState = rawget(_G, HUB_KEY)
...
function HubState.Stop()
    HubState.Active = false
    if HubState.Cleanup then
        pcall(HubState.Cleanup)
    end
...
local restoreCharacterCollisions

HubState.Cleanup = function()
    espEnabled = false
    noclipEnabled = false
    flyEnabled = false
    ...
    gui = gui and gui:FindFirstChild("IKAROHUB_CARROS")
...
local function toggleGhostMode()
    ...
    for _, v in pairs(player.Character:GetDescendants()) do
        if v:IsA("BasePart") or v:IsA("Decal") then
            if ghostTransparency[v] == nil then
                ghostTransparency[v] = v.Transparency
                v.Transparency = 1
            else
                v.Transparency = ghostTransparency[v]
                ghostTransparency[v] = nil
            end
        end
    end
end
...
restoreCharacterCollisions = function()
...
local function setupSimpleAlerts()
    local function monitorPlayer(plr)
        trackConnection(plr:GetPropertyChangedSignal("Team"):Connect(function()
...
local function createGUI()
    local existingGui = player.PlayerGui:FindFirstChild("IKAROHUB_CARROS")
    if existingGui then existingGui:Destroy() end
    
    local screenGui = Instance.new("ScreenGui")
    screenGui.Name = "IKAROHUB_CARROS"
                    ...
                    if #readyATMs > 0 then
                        for _, item in ipairs(readyATMs) do
                            ...
                            if atmCounter >= 12 and farmModeEnabled then
                                ...
                                -- Restore the exact collision values that existed before noclip.
                                restoreCharacterCollisions()
...
trackConnection(RunService.Stepped:Connect(function()
    ...
    elseif next(originalCollisions) then
        restoreCharacterCollisions()
    end
                ...
                if dist <= item.MaxDist then
                    ...
                    task.spawn(function()
                        ...
                        local ok, err = xpcall(function()
                            if not HubState.Active or not prompt.Parent or not hrp.Parent then return end
                            prompt.RequiresLineOfSight = false
                            hrp.Anchored = true
                            prompt:InputHoldBegin()
                            holdStarted = true
                            ...
                            local start = tick()
                            while HubState.Active and prompt.Parent and (tick() - start) < holdTime do
                                RunService.Heartbeat:Wait()
                            end

                            if not HubState.Active or not prompt.Parent then return end
                            prompt:InputHoldEnd()
                            holdStarted = false
                            task.wait(0.5)
                            completed = true
                        ...
                        if holdStarted and prompt and prompt.Parent then
                        ...
                        elseif not ok then
                            warn("IKAROHUB Auto Prompt: " .. tostring(err))
                        end
...
task.spawn(function()
    while HubState.Active do
        ...
        if not HubState.Active then break end
        pcall(function()
            if VirtualUser then
